...
{
    // Колекції для зберігання даних про монітори
    public List<LCDMonitor> LCDMonitors { get; private set; }
    public List<TouchScreenMonitor> TouchScreenMonitors { get; private set; }

    // Конструктор
    public MonitorManager()
    {
        LCDMonitors = new List<LCDMonitor>();
        TouchScreenMonitors = new List<TouchScreenMonitor>();
    }

    // Метод для додавання монітора до відповідної колекції
    public void AddMonitor(Monitor monitor)
    {
        if (monitor is LCDMonitor lcdMonitor)
        {
            LCDMonitors.Add(lcdMonitor);
        }
        else if (monitor is TouchScreenMonitor touchscreenMonitor)
        {
            TouchScreenMonitors.Add(touchscreenMonitor);
        }
    }
}
...
class Program
{
    ...
    static void Main(string[] args)
    {
        ...
        manager.AddMonitor(new LCDMonitor
        {
            Model = "VS239H-P",
            Manufacturer = "ASUS",
            Diagonal = 23,
        ...
        // Тепер ви можете отримати доступ до даних про монітори через відповідні колекції в об'єкті manager
        // Наприклад, для виведення інформації про всі сенсорні монітори:
        Console.WriteLine("Список сенсорних моніторів:");
        foreach (var touchscreenMonitor in manager.TouchScreenMonitors)
        {
            Console.WriteLine($"Модель: {touchscreenMonitor.Model}, Виробник: {touchscreenMonitor.Manufacturer}, Ціна: {touchscreenMonitor.Price}");
        }

        // Або для редагування першого LCD монітора:
        if (manager.LCDMonitors.Count > 0)
        {
            var firstLCDMonitor = manager.LCDMonitors[0];
            firstLCDMonitor.Price -= 5000; // Знижка на ціну першого LCD монітора
            Console.WriteLine($"Ціна першого LCD монітора після знижки: {firstLCDMonitor.Price}");
